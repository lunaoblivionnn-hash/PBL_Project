...
// --- PROSES UPDATE NILAI MANUAL OLEH GURU ---
...
// 2. Ambil Data Nilai Siswa
$q_nilai = mysqli_query($koneksi, "
    SELECT s.IDSiswa, s.NamaSiswa, kn.IDNilai, kn.WaktuMulai, kn.WaktuSelesai, kn.Benar, kn.Salah, kn.NilaiAkhir
    FROM siswa s
...
<body>
                ...
                <div class="col-md-3 col-6">
                    <div class="stat-card">
                        ...
                        <div class="fs-5 fw-bold <?= $kuis['Status'] == 'Published' ? 'text-success' : 'text-warning' ?>">
                            <?= $kuis['Status'] == 'Published' ? '<i class="bi bi-record-circle-fill me-1"></i> Sedang Berjalan' : '<i class="bi bi-check-circle-fill me-1"></i> Selesai (Diumumkan)' ?>
                        </div>
                    </div>
                </div>
                <div class="col-md-3 col-6">
                            ...
                            <tr class="item-siswa" data-nama="<?= strtolower($ds['NamaSiswa']) ?>" data-nilai="<?= is_null($nilai) ? -1 : $nilai ?>">
                                ...
                                <td class="text-center">
                                    <div class="d-flex align-items-center justify-content-center gap-2">
                                        ...
                                        <?php if($status == 'Selesai'): ?>
                                            <button class="btn btn-sm btn-outline-primary shadow-sm" type="button" data-bs-toggle="modal" data-bs-target="#modalReview<?= $ds['IDSiswa'] ?>" title="Review Jawaban Siswa"><i class="bi bi-search"></i></button>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="tab-pane fade" id="tab-analisis" role="tabpanel">
                <div class="row g-4 mt-1">
                    ...
                    while($soal = mysqli_fetch_assoc($q_soal)): 
                        ...
                        // Ambil daftar nama siswa yang benar dan salah
                        $q_analisis = mysqli_query($koneksi, "
                            SELECT s.NamaSiswa, j.IsBenar 
                            FROM kuis_jawaban j 
                            JOIN kuis_nilai kn ON j.IDNilai = kn.IDNilai 
                            JOIN siswa s ON kn.IDSiswa = s.IDSiswa
                            WHERE j.IDSoal='$id_s' AND kn.IDKuis='$id_kuis' AND s.Kelas='$kelas'
                        ");
                        $siswa_benar = [];
                        $siswa_salah = [];
                        while($an = mysqli_fetch_assoc($q_analisis)) {
                            if($an['IsBenar'] == 1) $siswa_benar[] = $an['NamaSiswa'];
                            else $siswa_salah[] = $an['NamaSiswa'];
                        }
                        $list_benar = empty($siswa_benar) ? '-' : implode(', ', $siswa_benar);
                        $list_salah = empty($siswa_salah) ? '-' : implode(', ', $siswa_salah);
                    ?>
                    <div class="col-md-6">
                        <div class="analysis-card h-100 shadow-sm" style="cursor: pointer; transition: transform 0.2s;" 
                             onmouseover="this.style.transform='translateY(-3px)';" 
                             onmouseout="this.style.transform='translateY(0)';"
                             onclick="showAnalisisDetail('<?= htmlspecialchars(addslashes($list_benar)) ?>', '<?= htmlspecialchars(addslashes($list_salah)) ?>', <?= $no ?>)">
                            ...
                            <div class="d-flex align-items-start gap-3">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="text-center mt-2 small text-muted"><i class="bi bi-hand-index-thumb"></i> Klik untuk melihat daftar siswa</div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body bg-light">
                    <?php
                    $id_nilai_siswa = $ds['IDNilai'];
                    $q_rev = mysqli_query($koneksi, "
                        SELECT s.*, j.JawabanTeks, j.IsBenar
                        FROM kuis_soal s 
                        LEFT JOIN kuis_jawaban j ON s.IDSoal = j.IDSoal AND j.IDNilai = '$id_nilai_siswa'
                    ...
                    <div class="card mb-3 border-0 shadow-sm <?= $rev['IsBenar'] ? 'border-start border-4 border-success' : 'border-start border-4 border-danger' ?>">
                        <div class="card-body">
                            ...
                            <?php if(!$is_essay): 
                                // KODE BARU: Mengambil teks opsi A,B,C,D dari database
                                $q_opsi = mysqli_query($koneksi, "SELECT * FROM kuis_opsi WHERE IDSoal='{$rev['IDSoal']}' ORDER BY IDOpsi ASC");
                                $jawaban_arr = explode(',', $teks_jawab);
                                $abjad = 'A';
                                $popover_content = "";
                                $jawaban_siswa_teks = [];
                                
                                while($op = mysqli_fetch_assoc($q_opsi)){
                                    $teks_opsi_bersih = htmlspecialchars($op['TeksOpsi']);
                                    $baris_opsi = "<b>{$abjad}.</b> {$teks_opsi_bersih}";
                                    if($op['IsBenar'] == 1) $baris_opsi .= " <span class='text-success'>(Kunci)</span>";
                                    
                                    $popover_content .= "<div class='mb-1 border-bottom pb-1'>{$baris_opsi}</div>";
                                    
                                    if(in_array($op['IDOpsi'], $jawaban_arr)) {
                                        $jawaban_siswa_teks[] = "<b>{$abjad}.</b> {$teks_opsi_bersih}";
                                    }
                                    $abjad++;
                                }
                                $tampil_jawaban = empty($jawaban_siswa_teks) ? '<i class="text-danger">Tidak dijawab</i>' : implode('<br>', $jawaban_siswa_teks);
                            ?>
                                <div class="text-muted small mb-1">Jawaban Siswa:</div>
                                <div class="p-2 border rounded <?= $rev['IsBenar'] ? 'bg-success bg-opacity-10 border-success' : 'bg-danger bg-opacity-10 border-danger' ?>" 
                                     data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-placement="top" data-bs-html="true" data-bs-title="Opsi Jawaban Lainnya" data-bs-content="<?= htmlspecialchars($popover_content) ?>" 
                                     style="cursor: pointer;">
                                    <div class="text-dark"><?= $tampil_jawaban ?></div>
                                    <div class="text-muted mt-2" style="font-size: 0.7rem;"><i class="bi bi-info-circle me-1"></i>Arahkan kursor untuk melihat opsi lain</div>
                                </div>
                                
                            <?php else: ?>
                                <div class="text-muted small mb-1">Jawaban Esai Siswa:</div>
                                <div class="p-2 bg-white border border-secondary-subtle rounded mb-2 <?= $rev['IsBenar'] ? 'text-success fw-bold' : '' ?>"><?= nl2br(htmlspecialchars($teks_jawab ?: 'Tidak dijawab')) ?></div>
                                <div class="text-muted small mb-1">Kunci Jawaban Guru:</div>
                                <div class="p-2 bg-success bg-opacity-10 border border-success border-opacity-25 rounded text-success fst-italic"><?= nl2br(htmlspecialchars($rev['KunciJawaban'] ?: 'Tidak ada kunci khusus')) ?></div>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php $no_rev++; endwhile; ?>
                ...
                <div class="modal-footer bg-white d-flex justify-content-between align-items-center border-top">
                    <form method="POST" class="d-flex align-items-center gap-2 m-0">
                        ...
                        <label class="fw-bold text-secondary mb-0">Revisi Nilai Akhir:</label>
                        <input type="number" name="nilai_manual" class="form-control text-center fw-bold fs-5" style="width: 100px; color:var(--primary);" value="<?= $ds['NilaiAkhir'] ?>" max="100" min="0" step="1">
                        <button type="submit" name="update_nilai_manual" class="btn btn-success fw-bold px-3"><i class="bi bi-save-fill me-1"></i> Simpan Nilai</button>
                    </form>
    ...
    <div class="modal fade" id="modalAnalisisDetail" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-light border-bottom-0 pb-2">
                    <h6 class="modal-title fw-bold text-dark" id="analisisModalTitle">Detail Soal</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body pt-0">
                    <div class="mb-3">
                        <div class="small fw-bold text-success mb-1"><i class="bi bi-check-circle-fill me-1"></i> Benar:</div>
                        <div class="p-2 bg-success bg-opacity-10 rounded text-dark small" id="listBenar" style="max-height: 120px; overflow-y: auto;"></div>
                    </div>
                    <div>
                        <div class="small fw-bold text-danger mb-1"><i class="bi bi-x-circle-fill me-1"></i> Salah:</div>
                        <div class="p-2 bg-danger bg-opacity-10 rounded text-dark small" id="listSalah" style="max-height: 120px; overflow-y: auto;"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Pemanggil Modal Analisis dan Popover (KODE BARU)
        function showAnalisisDetail(benar, salah, no) {
            document.getElementById('analisisModalTitle').innerText = 'Daftar Siswa (Soal No. ' + no + ')';
            document.getElementById('listBenar').innerText = benar;
            document.getElementById('listSalah').innerText = salah;
            new bootstrap.Modal(document.getElementById('modalAnalisisDetail')).show();
        }
