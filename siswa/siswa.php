...
if($mode_detail) {
    ...
    // Sinkronisasi dengan nama tabel database asli kamu: pengumpulan_tugas
...
=======
            <div class="d-flex align-items-center gap-3">
                <div class="text-end text-white d-none d-md-block">
                    <h6 class="mb-0 fw-bold small text-nowrap" style="font-size: 1.15rem"><?= htmlspecialchars($siswa['NamaSiswa'] ?? 'Siswa') ?></h6>
...
=======
                        ...
                        <li class="nav-item"><a class="nav-link active" href="tugas.php"><i class="bi bi-book me-2"></i>Mata Pelajaran</a></li>
                ...
                <div class="row g-4" id="courseContainer">
                    ...
                    <?php else: ?>
                        ...
                        while($row = mysqli_fetch_assoc($query_mapel)): 
                            ...
                            if(!empty($row['Gambar']) && file_exists("../image/mapel/" . $row['Gamb=ar'])) {
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card mapel-card p-4 mb-4">
                                ...
                                <p class="text-secondary" style="line-height: 1.6; font-size: 1.05rem;"><?= nl2br(htmlspecialchars($tugas['Deskripsi'])) ?></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card mapel-card p-4">
                                <h5 class="fw-bold text-dark mb-3"><i class="bi bi-cloud-arrow-up text-danger me-2"></i>Status & Pengumpulan Tugas</h5>
                                
                                <?php if($sudah_kumpul): ?>
                                    <div class="alert alert-success border-0 rounded-3 d-flex align-items-start gap-2 p-3 mb-3">
                                        <i class="bi bi-check-circle-fill mt-0.5 fs-5"></i>
                                        <div>
                                            <span class="fw-bold d-block">Tugas Telah Berhasil Dikumpulkan!</span>
                                            <span class="small opacity-75">Selesai dikirim pada: <?= date('d M Y, H:i', strtotime($kumpul['TanggalKirim'])) ?> WIB</span>
                                        </div>
                                    </div>
                                    <div class="p-3 bg-light rounded-3 mb-3">
                                        <small class="text-muted d-block mb-1 fw-medium">Berkas Jawaban Anda:</small>
                                        <a href="uploads_tugas/<?= htmlspecialchars($kumpul['FileJawaban']) ?>" target="_blank" class="text-decoration-none fw-semibold text-truncate d-block small text-danger">
                                            <i class="bi bi-file-earmark-check-fill me-1 fs-5"></i> Lihat Berkas Terkirim (<?= htmlspecialchars($kumpul['FileJawaban']) ?>)
                                        </a>
                                    </div>
                                    <div class="bg-light rounded-3 p-3 text-center">
                                        <span class="text-muted small d-block mb-1 fw-medium">Nilai Hasil Evaluasi Guru:</span>
                                        <h2 class="fw-bold text-success mb-0"><?= !empty($kumpul['Nilai']) ? $kumpul['Nilai'] : 'Belum Dinilai' ?></h2>
                                    </div>
                                <?php else: ?>
                                    <div class="alert alert-warning border-0 rounded-3 d-flex align-items-start gap-2 mb-4">
                                        <i class="bi bi-exclamation-circle-fill mt-0.5 fs-5"></i>
                                        <span class="fw-semibold">Kamu belum mengirimkan lembar berkas jawaban untuk tugas ini. Selesaikan sebelum batas waktu berakhir!</span>
                                    </div>
                                    
                                    <form action="proses_upTugas.php" method="POST" enctype="multipart/form-data" class="mt-2">
                                        <input type="hidden" name="id_tugas" value="<?= htmlspecialchars($id_tugas) ?>">
                                        
                                        <div class="mb-4">
                                            <label for="file_jawaban" class="form-label fw-bold text-dark">Pilih Berkas Lembar Jawaban (Format yang didukung: PDF/DOCX/ZIP/XLS):</label>
                                            <input class="form-control form-control-lg bg-light" type="file" id="file_jawaban" name="file_jawaban" required>
                                        </div>
                                        
                                        <button type="submit" name="submit_tugas" class="btn btn-danger text-white rounded-pill px-4 py-2.5 fw-semibold shadow-sm" style="background: linear-gradient(135deg, #dc3545, #9b1c26); border: none;">
                                            <i class="bi bi-send-fill me-1"></i> Kirim Jawaban Tugas Sekarang
                                        </button>
                                    </form>
                                <?php endif; ?>
                            </div>
